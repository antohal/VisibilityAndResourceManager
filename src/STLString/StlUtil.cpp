...
wstring FormatString(const wchar_t* in_pwstrFormat, ...)
{
	va_list args;
	...
	assert_debug(Res < dwBuf, L"неправильный format string или получилась слишком длинная строка");
	pwstrTmp[dwBuf - 1] = 0;
	va_end(args);
	return wstring(pwstrTmp);
}
