...
struct SGlobalTerrainShaderParams
{
	...
	// Разбиения по широте должны быть делителями числа nCountX-1
	// Разбиения по долготе должны быть делителями числа nCountY-1
	unsigned int	aPartitionCoefficients[20][2]; // первая координата - по широте, вторая координата - по долготе

	// Количество вершин по X и Y  для каждого уровня
	unsigned int	aVertexCounts[20][2];	// первая координата - по широте, вторая координата - по долготе
...
struct STerrainBlockShaderParams
{
	...
	unsigned int	nCountX = 0;					// количество точек по широте (может не совпадать с разрешением текстуры)
	unsigned int	nCountY = 0;					// количество точек по долготе (может не совпадать с разрешением текстуры)
